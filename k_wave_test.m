...
% Create 1 internal structure using makeDisk
discs = makeDisc(Nx, Ny, 75, 55, 5);
...
vmcmedium.scattering_coefficient = 20*ones(Nx, Ny);
vmcmedium.absorption_coefficient = 0.05*ones(Nx, Ny);
% Define the acoustic properties    

vmcmedium.absorption_coefficient(disc_indices) = 0.1;
...
vmcboundary.lightsource(boundary_with_lightsource) = {'direct'};
...
subplot(1,2,2)
imagesc(kgrid.x_vec*1e3, kgrid.y_vec*1e3, source.p0, [min(source.p0(:)) ...
        max(source.p0(:))]);
...
p_xy = kspaceLineRecon(sensor_data.', dy, kgrid.dt, medium.sound_speed(1), 'Plot', true);
% threshold = 0.5;
% max_pressure = max(p_xy(:));
% p_xy(p_xy<max_pressure*threshold) = 0;
% define a second k-space grid using the dimensions of p_xy
[Nx_recon, Ny_recon] = size(p_xy);

kgrid_recon = kWaveGrid(Nx_recon, kgrid.dt * medium.sound_speed(1), Ny_recon, dy);
...
imagesc(kgrid.x_vec*1e3, kgrid.y_vec*1e3,p_xy_rs);
colorbar
xlabel('x-position [mm]');
ylabel('y-position [mm]');
